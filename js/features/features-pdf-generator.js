...
async function generateQuestionsFromText(text, config) {
  ...
  try {
    ...
    // Vérifier que la clé API est présente (obligatoire)
    if (!config.apiKey || config.apiKey.trim().length === 0) {
      throw new Error('Clé API Mistral manquante. Veuillez fournir votre clé API dans les paramètres.');
    }
    ...
    const response = await fetch(BACKEND_URL, {
     ...
     body: JSON.stringify({
       ...
       apiKey: config.apiKey, // Clé API obligatoire
       metadata: {
